...
void LOC_voidDeclareOutputPins(u8 PinCounter);
...
void LOC_voidDeclareInputPins(u8 PinCounter);
...
void Port_Init(const Port_ConfigType *ConfigPtr)
{
	...
	for (u8 PinCounter = 0; PinCounter < PortNumberOfPortPins; PinCounter++)
	{
		...
		case PORT_PIN_MODE_DIO:
			...
			{
			case PORT_PIN_OUT:
				LOC_voidDeclareOutputPins(PinCounter);
				// PORT_PIN_OPEN_DRAIN
			...
			default: /* start of default case of switch(ConfigPtr[PinCounter].Port_PinInitialDirection) */
				...
				LOC_voidDeclareInputPins(PinCounter);
